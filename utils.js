function deepCopy(obj) {
  ...
  if (Array.isArray(obj)) {
    for (let i = 0, size = obj.length; i < size; i++) {
      if (obj[i] && obj[i].constructor && obj[i].constructor.name === 'Cell') {
        newObj[i] = Object.assign(Object.create(Object.getPrototypeOf(obj[i])), newObj[i]);
      }
    }
  }
  return newObj
...
class Cells {
  ...
  isAvailable(cell) {
    ...
    while (i < this.times.length && j < b.length) {
      if (this.times[i] == b[j]) return false
      else if (this.times[i] < b[j]) i++;
      else j++;
    }
    return true
  }
...
class TimeTables {
  ...
  push(obj) {
    if (!(obj instanceof TimeTable)) throw new TypeError("obj must be instance of TimeTable")
    this.arr.push(obj)
  }
...
function 초성찾기(list, search) {
  function is한글(str) { return /[ㄱ-ㅎ|ㄳ-ㅄ|가-힝]/.test(str) }
  function 초성(str) {
    if (is한글(str)) {
      ...
      for (let i = 0; i < str.length; i++) {
        ...
        if (Object.keys(겹).includes(c)) result += 겹[c]
        else if (초.includes(c)) result += c
